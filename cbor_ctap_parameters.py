...
pubKeyCredParams = cb_list(1, 8, PublicKeyCredentialType)

PublicKeyCredentialDescriptor = cb_map([
    {'key': 'type', 'type': cb_str(1, 32), 'required': True},
    {'key': 'id', 'type': cb_bytes(1, 256), 'required': True}
    # transports skipped
])
...
authenticatorMakeCredential = cb_map([
    ...
    {'key': 8, 'type': cb_bytes(16, 16), 'required': False},
    {'key': 9, 'type': cb_int(1, 1), 'required': False}
])
...
authenticatorGetAssertion = cb_map([
    ...
    {'key': 7, 'type': cb_int(1, 1), 'required': False}
])
